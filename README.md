...
- Responsive dark/light homepage and page/function-first VAPT operator console with a compact primary rail, sticky in-plan controls, prioritized scan rows, skeleton loading, filter chips, and keyboard shortcuts.
- One consistent product design across the homepage, console, methodology, help, and workflow pages: shared spacing, typography, surfaces, forms, focus states, responsive layouts, reduced-motion support, and smooth in-app/cross-document transitions.
- Performance-first rendering: the homepage never loads the 623-item catalog, attack-path previews load near the viewport, practical test bodies mount only when opened, off-screen long views use content visibility, and reporting tables render on demand.
...
- **Assessment-first operator console:** Setup starts with “What type of assessment are you performing?” — seven realistic VAPT presets (Static Website — Black Box, Web Application — Black/Grey Box, Grey Box with Multiple User Roles, Public-Facing App with Login and Registration, API Security Testing, Custom Assessment). Each preset assumes its initial context, then asks **only** the relevant follow-up questions in interview order; answers keep filtering later questions, and anything unconfirmed widens the plan instead of hiding work. The generated **Testing Plan** is grouped into collapsible attack-surface categories (TLS, HTTP, Client-Side, Authentication → JWT / Cookie & Session, Authorization, API, GraphQL, WebSocket, File Handling, Business Logic, AI), each stating **why it is included**, with a cross-role privilege matrix when multiple role tiers exist. “Edit context” adds discoveries mid-engagement (a hidden API, WebSocket, upload, another auth mechanism) and the plan expands without losing any progress. The **Applicable Test Count is catalog-derived** from Active + Confirm items; Core, Extended, and Specialized depth plus category tabs filter the same rows in place. Practical rows expand inline into Quick Test, named variants, payloads, tools, CHECK FOR, VALIDATE, DO NOT REPORT, and reasoned “because of this test, also check” links. Methodology-only work stays honest and opens the full methodology instead of receiving a fabricated request, and a page/function focus remains available as a secondary view.
- **Coverage families underneath the plan:** 196 validated families still cover every one of the 623 checks and power gap analysis, resume, evidence, and adjacency. They remain available as a reference/coverage view, but the tester does not need to choose between a family and a page plan to start working.
