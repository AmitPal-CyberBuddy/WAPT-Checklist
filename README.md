...
- **Page/function-first operator console:** Start with “What are you looking at?” (static page, login, reset, upload, API request, WebSocket, checkout, and more). The selected surface is remembered and becomes one compact **Testing Plan**, not a competing dashboard/playbook/family workflow. The **Applicable Test Count is catalog-derived** from Active + Confirm items, while the default working set is a practical 30-test Core pass for a static page. Core, Extended, and Specialized depth plus HTTP/Headers/TLS/Client-side tabs filter the same rows in place. Practical rows expand inline into Quick Test, named variants, payloads, tools, CHECK FOR, VALIDATE, DO NOT REPORT, and reasoned “because of this test, also check” links. Methodology-only work stays honest and opens the full methodology instead of receiving a fabricated request.
