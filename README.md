...
- **Page playbooks:** sixteen surface packs (static page, login, registration, password reset, profile, session, upload, search, checkout, admin, REST API, GraphQL, WebSocket, OAuth/SSO, JWT, SPA) that list every applicable check with named variants and copyable payloads — Host header (arbitrary, duplicate, X-Forwarded-Host), CSP, testssl, path traversal, BOLA, alg=none, redirect_uri, SVG upload, price tamper. Scoping a static site opens that playbook; every other scope lands on the matching board.
