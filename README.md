...
- **Context-driven assessment:** scope the application (static / login / API / …) and the dashboard becomes the test plan — only applicable checks, grouped by attack surface, each opening a Quick Test + variants + payloads + validation. Methodology, mappings, and remediation stay under Reporting and reference.
