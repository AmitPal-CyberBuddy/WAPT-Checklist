...
test('all user-facing pages share policy, branding, theme, and responsive viewport', () => {
  for (const page of ['index.html', 'app.html', 'methodology.html', 'docs.html', 'workflow.html']) {
    ...
    assert.ok(html.includes('css/polish.css?v=1.0.0-r11'), `${page} shared product polish`);
    assert.match(html, /src="js\/ui\/[^"]+\.js\?v=1\.0\.0-r11"/);
    assert.ok(html.includes('js/ui/theme-boot.js?v=1.0.0-r11'), `${page} early theme boot`);
