...
| Functionality testing (round 4) | All gates green | **Complete** | `tools/functional-workflows.mjs` runs the real app in jsdom with real HTTP/storage/events: 41/41 PASS; found+fixed 3 bugs (Phase 5 dashboard crash REGRESSION, stale-closure state clobber, evidence-stage refresh); `docs/FUNCTIONAL-TEST-REPORT.md` |
| Merge & deploy | Deployed | **Complete** | PR #3 merged 2026-08-18 (`9874987`); Deploy GitHub Pages workflow success; live site verified (release.json r6, families.json, app shell) |
