...
- **File-handling category is now 26/26 step-by-step** (19 new checks): acceptance/delivery contracts, filename normalization (extension roulette, Unicode, double extensions), isolated delivery origins with cookie-scope checks, overwrite/collision, archive symlink escapes, zip-bomb and entry-count bounds, quotas, decoded-image dimension bombs, normalization before processing, polyglot handling per reachable parser, AV-as-depth with EICAR pipeline proofs, attachment-target authorization, traversal encoding ladders, LFI/RFI/SSTI, Content-Disposition encoding, temp/quarantine accessibility, privileged-tool preview isolation, and generated-output validation (CSV formula injection, HTML-in-PDF, mail-merge).
- **Client-side category is now 37/63 step-by-step** (22 new checks): DOM source-to-sink mapping, eval/JSONP removal, HTML/CSS injection impact (dangling markup, attribute escapes, UI redressing), resource-URL manipulation, credentialed CORS behavior, postMessage origin validation, untrusted storage handling, IndexedDB identity/tenant partitioning, service-worker cache privacy, worker/broadcast-channel boundaries, prototype-pollution gadget chains, DOM clobbering, window.name hygiene, reverse tabnabbing, SRI, cross-site script inclusion, source-map review, Web Crypto key handling, XS-Leak channels, and bfcache/session-history state.
- **Plan tiers rebalanced for rich coverage**: Don't-miss now selects only critical/high work, and High-value keeps every critical plus the top five high-severity checks per category (practical work sorts first) — authored breadth flows into Extended instead of flooding the Core pass.


### Added (Practical-variant wave 1 continues: API security and GraphQL complete — r18)

- **API-security category is now 40/40 step-by-step** (~105 named variants across 35 previously reference-only checks): mutation, nested-resource, and batch-member authorization with alias-route parity; mobile-surface constraint across accounts and tenants; field filtering and protected-property updates across sparse/expanded/export representations; overfetch bounds to the operation contract; API keys scoped, revocable, and out of shipped bundles; token audience/type binding and cross-version authentication parity; page-size/window, body/array, and expensive-filter bounds; paid and async side-effect quotas; concurrent and distributed abuse controls keyed on identity; business-flow automation guards; function policy across methods, overrides, and route spellings; constrained role/membership management; deny-by-default for undocumented functions; workflow sequence and prerequisite enforcement; idempotency and one-time semantics; server-side fetch redirect/address revalidation (SSRF shapes); method/media-type hygiene; debug/stack/CORS/schema-console controls; and the inventory program — API host inventory, version retirement, shadow-API discovery, docs reconciliation, and consumed-API trust (webhook validation, upstream resilience, TLS identity, least-privileged credentials).
- **GraphQL category is now 15/15 step-by-step** (7 new checks): per-transport baselines (POST/GET/persisted/multipart/subscription with error-envelope snapshots), unauthorized mutation input fields including nested smugling and read-behind-mutation, query cost analysis, alias/fragment counting against complexity limits, pagination and cursor-forging bounds, subscription authorization at subscribe and event time with mid-stream revocation, and error hygiene without stacks, partial-data leaks, or private-schema suggestions.
- All schema, floor, content, and reference audits pass; a JWT+REST+GraphQL plan now renders 222 practical rows (API 40/40 in-plan, GraphQL 15/15).


### Added (Practical-variant wave 1 continues: authorization complete — r17)

- **Authorization category is now 43/43 step-by-step** (~105 named variants across 35 previously reference-only checks): the role-permission matrix as test oracle; horizontal delete with destructive-shape and bulk-smuggling probes; collection filtering including typeaheads/exports/totals; per-member bulk authorization; nested-resource parent-child chains; opaque references as inputs-not-secrets; direct media, report-artifact, and async-job authorization (status, cancel, results); hidden-UI-is-not-authorization; ownership reassignment via mass assignment; field-level read filtering and protected-property writes across representations; state-change rechecks and transition races; display-attribute independence; error/preview/metadata representations; audit views without payload leaks; tenant binding from trusted identity, cross-tenant object access, tenant-switch state clearing, and invitation/role-assignment controls; group membership and per-member roles; delegated access scope/expiry/revocation; workflow transition authorization; separation of duties (self-approval, approver choice, mid-flight swaps); revocation freshness across layers; stale self-contained role claims; batched-request per-operation checks; existence oracles; signed-URL surgery; realtime channel subscription/message authorization; confused-deputy integrations; deny-by-default for undocumented functions; layered admin authorization; and API-version consistency.
- All schema, floor, content, and reference audits pass; a many-role plan now renders the full authorization category practical (42 of 42 in-plan rows step-by-step; 158 practical rows plan-wide).
...
- **Clean URLs.** Every secondary page is now served at its extensionless address — `/app/`, `/methodology/`, `/docs/`, `/workflow/` — with rebased asset paths, canonical tags, and a sitemap listing the clean URLs. The old `.html` addresses remain as noindex redirect stubs that preserve query strings and share hashes (`/app.html#share/…` keeps working), and a friendly 404 page recovers extensionless typos. Verified end-to-end: the console boots from `/app/`, builds the plan, and round-trips share links.
- **No maintainer-facing copy in page view.** The public evidence-workflow document now tells testers where their data lives instead of listing implementation files; internal notes remain in the repository only.
- **Duplicate-content audit.** Across all 623 catalog items: no duplicate IDs, titles, or objectives. Two genuinely overlapping pairs were differentiated and cross-linked — GraphQL endpoint discovery (reconnaissance) vs. per-transport baseline (GraphQL suite), and single-challenge OTP guessing limits (authentication) vs. abuse across renewed challenges and channels (rate limiting). Intentional per-surface mirrors (API vs. web authorization, cookie vs. JWT vs. WebSocket token leakage) keep their distinct scopes and are linked as related work.

### Changed (Assessment-first setup, categorized plans, density & responsive pass — r10)

- **Assessment presets replace the page/function-first setup.** The Testing Plan flow now starts with “What type of assessment are you performing?” and seven realistic VAPT scenarios: Static Website — Black Box, Web Application — Black Box, Web Application — Grey Box, Grey Box with Multiple User Roles, Public-Facing Application with Login and Registration, API Security Testing, and Custom Assessment. Each preset states the context it already assumes (✓ publicly accessible, external attacker perspective, no authenticated access, …), then asks **only** its relevant follow-up questions in interview order (the public-login preset asks for the authentication mechanism first; the multi-role preset asks for the available role tiers first). Answers keep filtering later questions — discovering a login mid-assessment opens the identity questions — and anything left “Not confirmed yet” is listed at review and widens the plan instead of hiding work.
- **Answers drive the plan at mechanism level.** JWT categories appear only when JWT/bearer is selected, cookie/session attribute work only for cookie sessions, OAuth/SAML federation only when federation is selected — already enforced by the applicability engine, now surfaced honestly by the new setup flow and category reasons. A new `role_types` context attribute records which tiers exist (standard, privileged, administrator, support/internal, custom) and feeds a privilege ladder.
- **The generated plan is organized into meaningful, collapsible categories.** The dashboard plan groups applicable tests by attack-surface category (TLS, HTTP/Infrastructure, Client-Side, Authentication → JWT Security / Cookie & Session Security, Authorization, API, GraphQL, WebSocket, File Handling, Business Logic, AI/LLM). Every category header states **why it is included** (“Included because: JWT / token-based authentication was selected.”), and excluded categories are listed with the context that would add them.
- **Cross-role matrix.** With multiple roles or privilege tiers, the Authorization category gains a cross-role matrix (Administrator ↑ Privileged ↑ Standard) with the directions to walk: vertical, horizontal, function-level, and object-level (IDOR/tenant) access.
- **Edit context without losing progress.** A dashboard “Edit context” editor (auth mechanisms, identity capabilities, APIs & realtime, features, roles) lets testers add discoveries — a hidden API, WebSocket, file upload, another auth mechanism — and the plan expands automatically while every status, note, finding, and evidence pack stays attached. “Choose page / function” is demoted to an optional focus view under Reference & coverage.
- **Density pass.** The console reads as a testing workspace rather than a presentation page: tighter view/panel/metric padding, one-line scan rows, slimmer plan controls, wizard chrome, sidebar, and context editor, with a smaller 64px workspace header. Visual hierarchy, contrast tokens, and the marketing pages are untouched.
- **Responsive recomposition for tablet and phone.** Mobile is designed rather than shrunken: findings tables become stacked cards with labeled fields, plan controls unstick and reflow, quick-filters become a horizontally scrollable strip, presets and context groups stack, wizard footers respect safe areas, category-group headers wrap with their reasons, and coarse-pointer targets stay ≥44px.
- Bumped browser assets to `1.0.0-r10` so hosted clients receive the new flow immediately.
